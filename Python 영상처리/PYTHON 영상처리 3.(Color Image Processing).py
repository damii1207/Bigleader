...
#     # 메모리 할당
#     outImage = [[[0 for _ in range(outW)] for _ in range(outH)] for _ in range(3)]
#     value = askstring("파라볼라 값", "0~255")
#     ## ** 찐 영상처리 알고리즘 ** ##
#     for rgb in range(3) :
#         for i in range(inH) :
#             for k in range(inW) :
#                 outImage[rgb][i][k] = 255 - 255 * (inImage[rgb][i][k] / 127) ** 2
#
#     ##############################
#     displayImage()
#
#
# def cupImage():  # cap파라볼라
#     global window, canvas, paper, filename
#     global inImage, outImage, inH, inW, outH, outW
#     # 중요! 출력이미지의 크기를 결정 --> 알고리즘에 따라서...
#     outH = inH;    outW = inW;
...
#         for i in range(inH):
#             for k in range(inW):
#                 outImage[rgb][i][k] = 255 * (inImage[rgb][i][k] / 127) ** 2
...
image1Menu.add_command(label='감마', command=gammaImage)
# image1Menu.add_command(label='cap파라볼라', command=capImage)
# image1Menu.add_command(label='cup파라볼라', command=cupImage)
